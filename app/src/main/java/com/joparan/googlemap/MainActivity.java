...
public class MainActivity extends AppCompatActivity
    ...
    // ActivityCompat.requestPermissions를 사용한 퍼미션 요청을
    // 구별하기 위해 사용됩니다
...
//                    CurrentDestinationLocation currentDestinationLocation = new CurrentDestinationLocation();
//
//                    currentDestinationLocation.setCurrentLocation(currentPosition);
//                    currentDestinationLocation.setDestinationLocation(destinationPosition);

                    HashMap result = new HashMap<>();
                    result.put("Current Position", currentPosition);
                    result.put("Destination Position", destinationPosition);

                    writeNewUser("1",currentPosition,destinationPosition);
                }
            }
        });
    }
    ...
    private void writeNewUser(String userId, LatLng currentLocation, LatLng destinationLocation) {
        CurrentDestinationLocation userLocation = new CurrentDestinationLocation(currentLocation, destinationLocation);

        mDatabase.child("users").child(userId).setValue(userLocation)
